...
bilinear <- function(y_true, y_pred, alpha, D) {
  (1-alpha)*cross_entropy(y_true, y_pred) + alpha*(y_true*D + y_pred - y_true*y_pred*(1+D))
}
...
weighted_cross_entropy <- function(y_true, y_pred, D){
  -D*y_true*log(y_pred) - (1-y_true)*log(1-y_pred)
}

focal_loss <- function(y_true, y_pred, g){
  -y_true*(1-y_pred)^(g)*log(y_pred) - (1-y_true)*y_pred^(g)*log(1-y_pred)
}


cost_functions_comparison <- function(){
  ...
  bilinear_0 <- sapply(t, function(t){bilinear(0, t, 0.5, 5)})
  bilinear_1 <- sapply(t, function(t){bilinear(1, t, 0.5, 5)})
  bilinear_plot <- data.frame(t, bilinear_0, bilinear_1)
  
  focal_loss_0 <- sapply(t, function(t){focal_loss(0, t, 2)})
  focal_loss_1 <- sapply(t, function(t){focal_loss(1, t, 2)})
  focal_loss_plot <- data.frame(t, focal_loss_0, focal_loss_1)
  ...
  lbl_plot <- ggplot(focal_loss_plot, aes(x = t)) + 
    geom_line(aes(y = focal_loss_0)) + 
    geom_line(aes(y = focal_loss_1)) +
    xlab(TeX('\\hat{y}')) + 
    ylab('Value of loss function') +
    ggtitle('Focal loss function')
